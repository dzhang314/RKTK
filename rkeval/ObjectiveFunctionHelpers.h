#ifndef RKEVAL_OBJECTIVE_FUNCTION_HELPERS_H_INCLUDED
#define RKEVAL_OBJECTIVE_FUNCTION_HELPERS_H_INCLUDED
...
static inline void lrsm(mpfr_ptr dst, size_t dst_size, mpfr_srcptr mat) {
    for (size_t i = 0, k = 0; i < dst_size; ++i) {
...
static inline void lrss(mpfr_ptr dst_re, mpfr_ptr dst_du, size_t n,
                        mpfr_srcptr mat_re, size_t mat_di) {
...
static inline void elmm(mpfr_ptr dst, size_t n, mpfr_srcptr v, mpfr_srcptr w) {
    for (size_t i = 0; i < n; ++i) {
        mpfr_mul(dst + i, v + i, w + i, MPFR_RNDN);
    }
}

static inline void elmz(mpfr_ptr dst_re, mpfr_ptr dst_du, size_t n,
...
static inline void esqm(mpfr_ptr dst, size_t n, mpfr_srcptr v) {
    for (size_t i = 0; i < n; ++i) { mpfr_sqr(dst + i, v + i, MPFR_RNDN); }
}

static inline void esqz(mpfr_ptr dst_re, mpfr_ptr dst_du, size_t n,
                        mpfr_srcptr v_re, mpfr_srcptr v_du) {
    for (size_t i = 0; i < n; ++i) {
...
static inline void dotm(mpfr_ptr dst, size_t n, mpfr_srcptr v, mpfr_srcptr w) {
...
static inline void lvmm(mpfr_ptr dst, size_t dst_size, size_t mat_size,
                        mpfr_srcptr mat, mpfr_srcptr vec) {
    size_t skp = mat_size - dst_size;
    size_t idx = skp * (skp + 1) / 2 - 1;
    for (size_t i = 0; i < dst_size; ++i, idx += skp, ++skp) {
        dotm(dst + i, i + 1, mat + idx, vec);
    }
}

static inline void lvms(mpfr_ptr dst_re, mpfr_ptr dst_du,
                        size_t dst_size, size_t mat_size,
                        mpfr_srcptr mat_re, size_t mat_di,
    ...
    for (size_t i = 0; i < dst_size; ++i, idx += skp, ++skp) {
        ...
        if (idx <= mat_di && mat_di <= idx + i) {
            mpfr_add(dst_du + i, dst_du + i, vec_re + mat_di - idx, MPFR_RNDN);
        }
    }
}
...
static inline void resm(mpfr_ptr f, mpfr_ptr tmp, size_t n,
...
static inline void ress(mpfr_ptr dst, mpfr_ptr tmp_re, mpfr_ptr tmp_du,
                        ...
                        mpfr_srcptr x_re, size_t x_di, size_t x_offset,
                        mpfr_srcptr gamma) {
...
#endif // RKEVAL_OBJECTIVE_FUNCTION_HELPERS_H_INCLUDED
