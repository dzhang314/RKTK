...
function search(
    evaluator::RKOCProjectionEvaluator{Float64},
    order::Int, num_stages::Int, seed::UInt64
)
    tempname = @sprintf("RKTK-QR-%02d-%02d-XXXX-XXXX-XXXX-%016X.txt",
        order, num_stages, seed)
    @assert length(tempname) == 49
    if WRITE_FILE
        existing = find_existing_file(tempname[1:14], tempname[29:49])
    ...
    filename = @sprintf("RKTK-QR-%02d-%02d-%04d-%04d-%04d-%016X.txt", order,
...
function thread_work(order::Int, num_stages::Int, max_seed::UInt64)
    evaluator = RKOCProjectionEvaluator{Float64}(order, num_stages)
    while true
...
function main(order::Int, num_stages::Int, min_seed::UInt64, max_seed::UInt64)
    if WRITE_FILE
        dirname = @sprintf("RKTK-PROJECTION-SEARCH-%02d-%02d",
            order, num_stages)
